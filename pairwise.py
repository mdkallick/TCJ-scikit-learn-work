...
#          Mathias Kallick <mdkallick@gmail.com>
...
from sklearn.metrics.pairwise_fast import _chi2_kernel_fast, _sparse_manhattan
...
def custom_distances(distance_function, X, args=[], kwargs={},
                        Y=None, squared=False, symmetric=True):
    Y = np.copy(X)
    ...
    if(not symmetric):
        while not it.finished:
            #print(type(it.multi_index))
            tmp = distance_function(X[it.multi_index[0]],
                                           Y[it.multi_index[1]],
                                           *args, **kwargs)
            it[0][...] = tmp
            it.iternext()
        ...
        while not it.finished:
            # don't calculate results for lower triangle of array
            if(not (it.multi_index[0] > it.multi_index[1])):
                ...
                it[0][...] = tmp
            it.iternext()
        i_lower = np.tril_indices(distances.shape[0], -1)
...
if __name__ == '__main__':
    ...
    X = np.array(['hello', 'goodbye', 'goodness', 'h'])
    print(custom_distances(jaccard_distance, X))
